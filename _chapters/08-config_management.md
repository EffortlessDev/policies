...
1. All changes to production systems, network devices, and firewalls are approved by the EA CTO before they are implemented to assure they comply with business and security requirements.
...
1. Provisioning the first Salt server for a production pod requires bootstrapping Salt. The CTO will oversee provisioning a new Salt server.
